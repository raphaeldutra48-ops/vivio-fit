...
export default function ModelosDeAnamnese() {
  ...
  const problemas = perguntas.flatMap((p, i) => {
  ...
  const podeSalvar = nome.trim().length >= 2 && perguntas.length > 0 && problemas.length === 0;
  ...
  async function salvar() {
    ...
    const corpo = {
      ...
      perguntas: perguntas.map((p) => ({
        ...p,
        opcoes: TIPOS_COM_OPCOES.includes(p.tipo) ? p.opcoes.filter(Boolean) : [],
